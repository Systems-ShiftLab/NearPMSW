...
conn *conn_new(const int sfd, enum conn_states init_state,
	...
	/*   uint64_t* tmp = (uint64_t*)pool;//pmemobj_direct(root);
	    //printf( "%ld\n",PMEM_OBJ_POOL_UNUSED2_SIZE);
	    //printf( "%ld %ld %ld\n",sizeof(PMEMobjpool),sizeof(uint16_t),sizeof(void*));
	    printf("vaddr %p pmemobjid %x\n",pool,iteration);
...
static int read_into_chunked_item(conn *c) {
    ...
    startCycles = getCycle();
                    ...
                    tid =  c->objid & 0x3f;
                    //printf("%d %d\n",tid, tx->pop->run_id);
                    cmd_issue( 2, 0, tid, c->objid, c->rcurr, tocopy, ndp_device); 
                    //printf("called\n");   
#ifdef LOGTIME
            endCycles = getCycle();
            totalCycles = endCycles - startCycles;
       
            double totTime = ((double)totalCycles)/200000000;
            printf("ulog %f\n", totTime);
#endif                 
                }
#endif
