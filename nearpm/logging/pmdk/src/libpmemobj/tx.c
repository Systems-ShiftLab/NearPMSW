...
static void wait_dma_complete(struct tx *tx){
	...
	int it =0;
	while(((*((uint32_t*)(tx->pop->p_ops.device))) & 1) != 0){
...
pmemobj_tx_add_range_direct(const void *ptr, size_t size)
{
	...
	struct tx *tx = get_tx();
	//printf("%d %d\n",tid, tx->pop->run_id);
	cmd_issue( 2, 0, tid, tx->pop->run_id, ptr, size, tx->pop->p_ops.device);
	
	#ifdef GET_BREAKDOWN
	endCycles = getCycle();
	totalcycles = endCycles - startCycles;
	totalccls += totalcycles;
	double totTime = ((double)totalcycles)/2000000000;
	printf("cycles = %ld\n",totalccls);
	printf("timelog = %f\n", ((double)totalccls)/200000000);
	#endif
	return 0;
}
...
pmemobj_tx_add_range(PMEMoid oid, uint64_t hoff, size_t size)
{
	...
	struct tx *tx = get_tx();
	//printf("%d %d\n",tid, tx->pop->run_id);
	cmd_issue( 2, 0, tid, tx->pop->run_id, pmemobj_direct(oid) , size, tx->pop->p_ops.device);
	#ifdef GET_BREAKDOWN
	endCycles = getCycle();
	totalcycles = endCycles - startCycles;
	totalccls += totalcycles;
