...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {
        ...
        *tmp = (uint64_t)tmp;
        pmem_persist(tmp,64);
        uint32_t   tid,tidorig;
       
        //tid = iteration & 0x3f;
        tid = 0 & 0x3f;
        printf("tid %d %d vaddr %p pmemobjid %lx\n",tid, tidorig,pool,iteration);
        //tid = (tid<< 4)| iteration;
        tid = (tid<< 4)| 0;
        ...
        *tmp = tid;
        ...
        c->rbuf = (char *)pool;//(char *)malloc((size_t)c->rsize);
...
static int build_udp_headers(conn *c) {
    ...
    if (c->msgused > c->hdrsize) {
        void *new_hdrbuf;
        if (c->hdrbuf) {
            //printf("rbuf assign 3\n");
            new_hdrbuf = realloc(c->hdrbuf, c->msgused * 2 * UDP_HEADER_SIZE);
        } else {
            //printf("rbuf assign 4\n");
    ...
    if (s_it->it_flags & ITEM_CHUNKED) {
        ...
        while (sch && dch && remain) {
            ...
            memcpy(dch->data + dch->used, sch->data + copied, todo);
                ...
                printf("dest %p src %p\n",dch->data + dch->used, sch->data + copied);
            }
#endif
            dch->used += todo;
            copied += todo;
        ...
        while (len > done && dch) {
            ...
            memcpy(dch->data + dch->used, ITEM_data(s_it) + done, todo);
                ...
                printf("dest %p src %p\n",dch->data + dch->used, ITEM_data(s_it) + done);
            }
#endif
            done += todo;
            dch->used += todo;
    ...
    if (comm == NREAD_APPEND) {
        if (new_it->it_flags & ITEM_CHUNKED) {
            ...
            if (new_it->it_flags & ITEM_PSLAB) {
                ...
                printf("dest %p src %p\n",ITEM_data(new_it), ITEM_data(old_it));
                printf("dest %p src %p\n",ITEM_data(new_it) + old_it->nbytes - 2 /* CRLF */, ITEM_data(add_it));
                return 0;
            }
#endif
        ...
        if (new_it->it_flags & ITEM_CHUNKED) {
            ...
            if (new_it->it_flags & ITEM_PSLAB) {
                ...
                printf("dest %p src %p\n",ITEM_data(new_it), ITEM_data(add_it));
                printf("dest %p src %p\n",ITEM_data(new_it) + add_it->nbytes - 2 /* CRLF */, ITEM_data(old_it));
                return 0;
            }
#endif
...
static void setpage(void * addr){
	uint64_t pageNo = ((uint64_t)addr)/GRANULARITY;
	unsigned long * pageStart = (unsigned long *)(pageNo*GRANULARITY);
	mprotect(pageStart, GRANULARITY, PROT_READ);
	return;
}
...
//    printf("test1\n");
    ...
    if (siginfo->si_code == SEGV_ACCERR) {
		mprotect(pageStart, GRANULARITY, PROT_READ|PROT_WRITE);
       //printf("test1\n");
		if(all_updates >= 5 || updated_page_count == 50){
			for(int i=0;i<updated_page_count;i++){
                cp_count++;
				memcpy(checkpoint_start + i*GRANULARITY, page[i],GRANULARITY);
				pmem_persist( checkpoint_start + i*GRANULARITY,GRANULARITY);	 
                //cmd_issue(2,0,0,0, (uint64_t)(checkpoint_start + i*GRANULARITY),GRANULARITY,device);
//*/
...
#ifdef PSLAB
    ndp_device =  open_device("/sys/devices/pci0000:00/0000:00:00.2/iommu/ivhd0/devices/0000:0a:00.0/resource0");
    if (pslab_file) {
